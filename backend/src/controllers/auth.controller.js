...
export async function signup(req, res) {
  ...
  try {
    ...
    if (password.length < 6) {
      return res
        .status(400)
        .json({ message: "Contrasena con mas de  6 digitos requerida!" });
    }
    ...
    if (existingUser) {
      return res
        .status(400)
        .json({
          message: " Email is already in use, please try with another one.",
        });
    }
...
export async function login(req, res) {
  try {
    ...
    if (!user)
      return res.status(401).json({ message: "Email o Password no validos." });
...
export async function onboard(req, res) {
  try {
    ...
    try {
      ...
      console.log(
        "Strem user updated after onboarding  for " + updatedUser.fullName
      );
    } catch (streamError) {
      console.log(
